...
if(strtoupper(php_sapi_name()) == 'CLI')
{
	...
	while ($row = $mysqli->fetch_assoc($result)) {
		...
		if($row['membership_type'] == 'Single' || $row['membership_type'] == 'Family' && $row['family_head'] == '1'){
			...
			$subject = 'Welcome to Evosapiens Movement! Membership Expiring today | '.$member_id.'';
				 
						$body = '<p>Hello ' . $name . ',</p>';

						$body .= '<p>We hope you have enjoyed being a member of Evosapiens Movement. We would like to inform you that your membership will expire today.</p>';

						$body .= '<p>To continue your fitness journey with us, please renew your membership. You can visit our reception desk today, and our staff will be happy to assist you in selecting a new membership plan that suits your needs.</p>';
						...
						$fromName = "Evosapiens Movement";
						 ...
						 // Log Email
						 $mysqli->executeQry("INSERT INTO ".MESSAGE_LOGS." SET member_id = '".$id."', type='email', message_type='expiry_today', status='".$emailStatus."', message='".addslashes(strip_tags($subject))."', provider_response='".$emailStatus."', created_at=NOW()");

						 // WhatsApp
						 $waMessage = "Hello ".$name.", your membership expires today (".$end_date."). Renew your fitness plan at Evosapiens Movement.";
						 $waOk = $mysqli->sendWhatsAppMessage($mobile, $waMessage);
						 $waStatus = $waOk ? 'sent' : 'failed';
						 $mysqli->executeQry("INSERT INTO ".MESSAGE_LOGS." SET member_id = '".$id."', type='whatsapp', message_type='expiry_today', status='".$waStatus."', message='".addslashes($waMessage)."', provider_response='".($waOk ? 'ok' : 'not configured / failed')."', created_at=NOW()");
