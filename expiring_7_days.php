...
if(strtoupper(php_sapi_name()) == 'CLI')
{
	...
	error_log("expiring 7 report Started:".$start_time." ----- expiring 7 report Ended:" .date('Y-m-d H:i:s')."" . PHP_EOL."\r\n", 3, 'expiring7Report.txt');

	$logid = $mysqli->Resquest_Response_log("", strtoupper('expiry_mail'), '', 'mail to expiring memberships (+7)', ''); 
	$cur_date = date('Y-m-d');
	$target_date = date('Y-m-d', strtotime('+7 days', strtotime($cur_date)));
	...
	while ($row = $mysqli->fetch_assoc($result)) {
		...
		if($row['membership_type'] == 'Single' || $row['membership_type'] == 'Family' && $row['family_head'] == '1'){
			...
			$subject = 'Welcome to Evosapiens Movement! Membership Expiring in 7 days | '.$member_id.'';

			$body = '<p>Hello ' . $name . ',</p>';
			$body .= '<p>We hope you have enjoyed being a member of Evosapiens Movement. We would like to inform you that your membership will expire in 7 days, on ' . $end_date . '.</p>';
			...
			$mysqli->executeQry("INSERT INTO ".MESSAGE_LOGS." SET member_id = '".$id."', type='email', message_type='expiry_7d', status='".$emailStatus."', message='".addslashes(strip_tags($subject))."', provider_response='".$emailStatus."', created_at=NOW()");

			$waMessage = "Hello ".$name.", your membership expires in 7 days (".$end_date."). Renew your fitness plan at Evosapiens Movement.";
			$waOk = $mysqli->sendWhatsAppMessage($mobile, $waMessage);
			$waStatus = $waOk ? 'sent' : 'failed';
			$mysqli->executeQry("INSERT INTO ".MESSAGE_LOGS." SET member_id = '".$id."', type='whatsapp', message_type='expiry_7d', status='".$waStatus."', message='".addslashes($waMessage)."', provider_response='".($waOk ? 'ok' : 'not configured / failed')."', created_at=NOW()");
